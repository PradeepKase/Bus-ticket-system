...
public class CustomerDoaImpl implements CustomerDao{
	...
	public String bookTicket(String bName, int cusId, int no) throws BusException {
		...
		try (Connection con = BusUtility.provideConnection()){
			...
			PreparedStatement ps = con.prepareStatement("select * from bus where bName = ?");
			...
			if (rs.next()) {
				...
				PreparedStatement ps1 = con.prepareStatement("select datediff(?,current_date()) as date");
				...
				if (days <= 0) {
					...
					PreparedStatement ps2 = con.prepareStatement("insert into booking(cusId, busNo, seatFrom, seatTo) values (?, ?, ?, ?)");
					ps2.setInt(1, cusId);
					ps2.setInt(2, busNo);
					...
					if (x > 0) {
						...
						PreparedStatement ps3 = con.prepareStatement("update bus set availseats = ? where busNo = ?");
...
boolean flag = false;
		...
		try(Connection con = BusUtility.provideConnection()){
			PreparedStatement ps1 = con.prepareStatement("select * from booking where cusId = ?");
			ps1.setInt(1, cusId);
...
String message = "Ticket cancellation failed";
		...
		try (Connection con = BusUtility.provideConnection()){
				...
				if (rs.next()) {
					...
					PreparedStatement ps1 = con.prepareStatement("select * from booking where busNo = ? and cusId = ?");
					ps1.setInt(1, busNo);
					ps1.setInt(2, cusId);
				    ...
				    if (flag) {
						...
						PreparedStatement ps2 = con.prepareStatement("delete from booking where busNo = ? and cusId = ?");
						ps2.setInt(1, busNo);
						ps2.setInt(2, cusId);
